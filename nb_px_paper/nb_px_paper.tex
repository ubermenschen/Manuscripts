...
To address this need, a  stack  of thin Nb 
% , Cu, and Al monitor 
foils was irradiated with the 100 MeV proton beam at  Los Alamos National Laboratory's Isotope Production Facility was irradiated  to investigate the \ce{^{93}Nb}(p,4n)\ce{^{90}Mo} nuclear reaction as a  monitor for intermediate energy proton experiments and to benchmark state-of-the-art reaction model codes.
...
This work reports the first measurement of the  \ce{^{nat}Nb}(p,x)\ce{^{82m}Rb} reaction, and of the independent cross sections for    
...
% }
% \textred{
To avoid inadvertently reporting enhanced proton fluence due to \ce{^{nat}Si}(p,x)\ce{^{22,24}Na} contamination, the use of silicone adhesives for aluminum monitor foils is strongly discouraged.
% }
...
Accurate cross section measurements using the activation technique benefit from well-characterized monitor reactions, which  currently a paucity of such data at intermediate energies or have high uncertainties (\textgreater15\%).
...
% IPF
the Isotope Production Facility (IPF) at  Los Alamos National Laboratory, or the
% iThemba LABS
...
% a number of 
intense and distinct gamma-rays, 
% with high intensities,  
which can be used to uniquely identify it during post-activation assay.  
...
In addition, cumulative cross sections will be reported whenever it is impossible to use decay spectrometry to distinguish direct production of a nucleus from decay feeding.
