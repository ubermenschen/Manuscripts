...
%%
\title{(n,p) Cross-Section Measurements for \ce{^{64}Cu} and \ce{^{47}Sc} via d-D Neutron Generator}
...
\address[ucb]{Department of Nuclear Engineering, The University of California, Berkeley, Etcheverry Hall, 2521 Hearst Ave, Berkeley, CA 94709}
...
\comment{Feel free to suggest a better title.}


\comment{Need to think of any other people to be added as authors. Also, is this the proper ordering - first author, then alphabetical?}

\comment{High-level comment: is active or passive voice preferred in this field?}


Neutron activation cross sections for the \ce{^{47}Ti}(n,p)\ce{^{47}Sc} and \ce{^{64}Zn}(n,p)\ce{^{64}Cu} production reactions have been measured at the UC Berkeley High Flux Neutron Generator, for quasi-monoenergetic 2.7645 MeV incident neutrons. Measurements were performed using activation ratio methods, measured relative to the \ce{^{113}In}(n,n')\ce{^{113m}In} and \ce{^{115}In}(n,n')\ce{^{115m}In} inelastic scattering reactions. The use of thin foils and high-precision gamma spectroscopy using HPGe detectors allows for precision cross section measurements, to within 6\% uncertainty. These cross section measurements show strong agreement with literature and theoretical values, but with lower uncertainty than existing measurements. This work has also provided characterization of the neutron energy distributions available at the  UC Berkeley High Flux Neutron Generator, which can be used for other low-energy neutron reaction cross section measurements in the future. 
