...
The D(D,n)\ce{^3He} reaction at 100 keV lab energy produces neutrons with energies ranging from 2.2 to 2.85 MeV, over an angular range of 0-180\degree\ in the lab frame-of-reference with respect to the incident deuteron beam. This distribution has been well documented \cite{Liskien_Paulsen_1973} and is shown in \autoref{fig:scatt_angle} for 100 keV incident deuteron energy.
...
Since the samples are separated by only  8 mm from the DD reaction surface they  subtend a fairly significant  (~28\degree) angular range and experience an impressive  (approximately $\sci{1.3}{7}$neutrons/cm$^2$ s) neutron flux. This stands in contrast to other measurements which feature collimated beams and significantly lower total neutron flux.  
...
\comment{Need to replace this figure w/ non-capitalized labels - this figure needs significant work, and I would appreciate anyone who can provide a better one}
...
\comment{Change \autoref{fig:frac_plot} text from the $\pm 1\sigma$ to 68.2\%}
...
As shown in \autoref{sec:sample_loading} above, the effective neutron energy depends on the angle range subtended by the sample with respect to the incident deuteron beam.  In order to determine this angle it is necessary to measure the lateral location of the beam with respect to the sample location.  This centroid of the beam was measured using a 3 x 3 array of 0.5 cm diameter indium foils.  The relative activity of these foils was then determined via post-irradiation counting of the \ce{^{115m}In} isomer ($t_{1/2}$ = 4.486 h).  \autoref{fig:in_heat_plot} below shows the measured activities for these 9 indium foils.  Based on these values we are able to verify that the beam was indeed centered on the middle of the zinc and titanium samples.   

% 
% \begin{figure}
%  \centering
%  \includegraphics[scale=0.6]{./figures/XXX.pdf}
%  % fracplot.pdf: 372x295 pixel, 72dpi, 13.12x10.41 cm, bb=0 0 372 295
...
\comment{cire relevant NDS article for t1/2}


\comment{CHECK WITH ERIC for 3x3 In info and plot}
...
In addition to the \ce{^{115}In}(n,n')\ce{^{115m}In} reference cross section, the \ce{^{115}In}(n,$\gamma$)\ce{^{116m}In}  ($t_{1/2}$ = 54.29 min) activity can be used to determine the  \ce{^{64}Zn}(n,p) and \ce{^{47}Ti}(n,p) cross section.  The capture activity is potentially subject to contamination from lower energy, especially thermal, \enquote{room return} neutrons since the (n,$\gamma$) cross section at 25 meV is approximately YYY MeV greater than at 2.7 MeV.    

\comment{cite relevant NDS article}

\comment{add appropriate numbers to (n,g) cross sections}

\comment{cite IRDFF somewhere?}
