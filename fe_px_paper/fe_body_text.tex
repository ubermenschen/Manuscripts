...
In contrast, \ce{^{51}Mn} ($t_{1/2}$ = 46.2$~\pm~$0.1\,min, $I_{\beta^+}$ = 96.86\%, $E_{\beta\text{, avg}}$ = 0.964\,MeV\,\cite{PhysRevC.96.014613,Wang2017}), 
...
Two separate irradiations were performed at incident proton energies of 25 and 55 MeV, respectively.
...
The target box
% , seen in \autoref{fig:fe_target_stack}, 
...
\begin{figure*}
 ...
 \caption{A gamma spectrum  from an activated Fe foil at approximately 55\,MeV (the maximum incident proton energy), collected 25 minutes after end-of-bombardment. Several observed reaction products are visible in this spectrum, and  the \ce{^{51}Mn} and \ce{^{52m}Mn} decay lines, which form two of the   primary reaction channels of interest, are  clearly isolated from surrounding peaks. }
...
The International Atomic Energy Agency (IAEA)-recommended \ce{^{nat}Ti}(p,x)\ce{^{46}Sc}, \ce{^{nat}Ti}(p,x)\ce{^{48}V},  \ce{^{nat}Cu}(p,x)\ce{^{62}Zn}, and \ce{^{nat}Cu}(p,x)\ce{^{63}Zn} monitor reactions were used 
...
The \enquote{variance minimization} techniques utilized in our recent work and established by Graves \etal\ have been used here to reduce uncertainty in proton fluence assignments due to poorly-characterized stopping power\,\cite{Voyles2018a,Graves2016}.
...
In performing the  minimization, the areal density of each of the  aluminum degraders (for the 55\,MeV stack)  was varied uniformly in FLUKA simulations  by a factor of up to $\pm$25\% of measured
% nominal 
...
% true areal densities of
for the stack components 
% differs from nominally measured values 
...
nominal 
FLUKA simulations,
% , and approximately 2\,MeV lower than 
% nominal 
% A\&Z calculations, both of 
which used the 
% nominal 
...
% , and approximately 5\,MeV lower than nominal A\&Z calculations.
...
The correlated uncertainty-weighted mean  for the two \ce{^{nat}Cu}(p,x) and two \ce{^{nat}Ti}(p,x) monitor channels was calculated at each energy position, to determine the final fluence assignments for the Cu and Ti foils, respectively, and the correlated uncertainty-weighted mean  for all four monitor channels was used to determine the final fluence assignments for the Fe foils.
...
% $\Sigma_{\beta_{ij}}$ is the correlation matrix for each $\beta$. 
...
% radioactive 
unstable ground states in these mass regions,  independent measurements of isomer-to-ground-state branching ratios for \ce{^{nat}Fe}(p,x)\ce{^{52m/g}Mn}, \ce{^{nat}Fe}(p,x)\ce{^{58m/g}Co}, and \ce{^{nat}Ti}(p,x)\ce{^{44m/g}Sc} were  extracted and are presented in \autoref{tab:fe_ibr_table}.
...
% in order to assess their predictive capabilities for the casual user.
% The explicit use of default parameters was chosen for two reasons --- 
%
...
% The 
It should be noted that the use of natural abundance targets in these measurements exacerbates the modeling  challenges seen in our previous work using monoisotopic targets\,\cite{Voyles2018a} 
% by propagating the difficulties inherent in modeling a wide 
due to the larger number  of reaction channels that can contribute to the production of any given residual nucleus.
% , to multiple target isotopes.
...
\begin{tabular}{ c c c c c}
 ...
 \underline{Code Version} & \underline{Proton/Neutron Optical Model} & \underline{Alpha Optical Model}  & \underline{E1 $\gamma$SF Model} & \underline{Level Density Model} \\ 
 EMPIRE-3.2.3\cite{Herman2007}       & Koning-Delaroche\cite{Koning2003} & Avrigeanu(2009)\cite{Avrigeanu2009}       & Modified Lorentzian\cite{belgya2006handbook} &  Enhanced Generalized Superfluid\cite{Giardina2002}. \\  
 TALYS-1.8\cite{Koning2012}          & Koning-Delaroche & Specific folded potential\cite{Koning2012}      & Brink-Axel Lorentzian\cite{Koning2012}  & Gilbert-Cameron\cite{Gilbert2011} \\
  CoH-3.5.3\cite{kawano2003coh,KAWANO2010} & Koning-Delaroche     & Avrigeanu(1994)\cite{Avrigeanu1994} & Generalized Lorentzian\cite{kawano2003coh,KAWANO2010} &  Gilbert-Cameron  \\
  ALICE-2017\cite{Blann1996}     &   Nadasen\cite{Nadasen1981}    &  Parabolic Diffuse-Well\cite{Thomas1959}    &   Berman-Fultz Lorentzian\cite{Berman1975a} & Kataria-Rarnamurthy-Kapoor\cite{Kataria1978,Kataria1990}
...
\begin{figure}
 \centering
 \includegraphics[width=0.65\textwidth]{./figures/Fe_Physical_Yields.pdf}
 % Nb_ptallies.pdf: 380x298 pixel, 72dpi, 13.41x10.51 cm, bb=0 0 380 298
 \caption{Integral thick target yields for \ce{^{nat}Fe}(p,x)\ce{^{51,52m,52g,54}Mn} and \ce{^{nat}Fe}(p,x)\ce{^{55,56,57,58m}Mn}, including indirect production routes. Yields are specified in units of MBq/C \cite{Otuka2015}, and  represent physical yields (also known as instantaneous production rates). 1 MBq/C= 0.0036 MBq/$\mmicro$Ah.}
 \label{fig:yields}
\end{figure}
...
The 
% various 
\ce{^{nat}Fe}(p,x), \ce{^{nat}Cu}(p,x), and \ce{^{nat}Ti}(p,x) cross sections measured here are in excellent agreement with 
...
% No cross sections for \ce{^{nat}Si}(p,x)\ce{^{22,24}Na} are reported due to the significant uncertainty in characterizing the layer of silicone adhesive, but t
This serves as another example of how the use of silicone-based adhesives may systematically enhance the apparent fluence if one were to use the \ce{^{nat}Al}(p,x)\ce{^{22,24}Na} monitor reactions.
Discussion of notable reaction channels follows here and 
comparisons  of the
remaining measured cross sections 
% and isomer branching ratios 
...
These curves show that production of medically-valuable radionuclides at low-to-intermediate energies by \ce{^{nat}Fe}(p,x) is potentially viable, but is dependent upon  the required radionuclidic purity on a case-by-case basis.
...
% discussion should follow the pattern:
% short statements about the comparison to earlier work by channel, short statements
% 4about the agreement with models by channel; overall agreement to earlier data and to particular models.



\subsection{\ce{^{nat}Fe}(p,x)\ce{^{51}Mn} cross section}

This work presents the first measurement of the \ce{^{nat}Fe}(p,x)\ce{^{51}Mn} reaction in this energy region.
However, below the 24.1\,MeV threshold for \ce{^{56}Fe}(p,x)\ce{^{51}Mn}, the single measurement of \ce{^{54}Fe}(p,x)\ce{^{51}Mn} from Levkovski can be renormalized by isotopic abundance (and an additional -18\% due to an error in the monitor data adopted in the measurement), and is consistent  with the present work \cite{levkovski1991cross}.
For the \ce{^{nat}Fe}(p,x)\ce{^{51}Mn} reactions, seen in \autoref{fig:51Mn}, the lower-energy  \ce{^{54}Fe}(p,$\alpha$)\ce{^{51}Mn} reaction, which peaks around 15\,MeV, is  well-modeled by TALYS and TENDL and over-predicted by the default calculation performed with  EMPIRE, CoH, and ALICE.
...
It should be noted that  \ce{Fe}(p,x)\ce{^{51}Mn} appears to offer a compelling alternative to the more established \ce{^{50}Cr}(d,x)\ce{^{51}Mn} pathway, which necessitates an enriched \ce{^{50}Cr} target to avoid radio-manganese contamination from reactions on stable isotopes of Cr\,\cite{Klein2000}.
% However, 
...
\subsection{\ce{^{nat}Fe}(p,x)\ce{^{52m,g}Mn} cross sections}
...
\begin{figure*}
    ...
    \subfloat{
        \centering
        \hspace{-5pt}\subfigimg[width=0.485\textwidth]{\hspace{4.5cm}a)}{./figures/52Mn.pdf}{50}
%         \caption{ Decay curve for the isomeric transition of \ce{^{115m}In}.}
         %         \refstepcounter{subfigure}
         \label{fig:temp_52Mn}
   \hspace{-5pt}}%
     \subfloat{
        ...
        \subfigimg[width=0.485\textwidth]{b)}{./figures/52Mn_IBR.pdf}{50}
         %         \refstepcounter{subfigure}
         \label{fig:temp_52mMn}
   \hspace{-5pt}}%
    \caption{a) Measured \ce{^{nat}Fe}(p,x)\ce{^{52}Mn} cross section, with \ce{^{56}Fe}(p,$\alpha$n)\ce{^{52}Mn}/\ce{^{54}Fe}(p,2pn)\ce{^{52}Mn} reaction channels visibly peaking at approximately \mbox{30 MeV}.   b) Measured  \ce{^{52\text{m}}Mn} ($2^+$) / \ce{^{52\text{g}}Mn}  ($6^+$)  isomer branching ratios via \ce{^{nat}Fe}(p,x).} 
     \label{fig:discussion_52Mn}
\end{figure*}

% Measured \ce{^{58\text{m}}Co} ($5^+$) / \ce{^{58\text{g}}Co}  ($2^+$)  isomer branching ratios via \ce{^{nat}Fe}(p,x), with the \ce{^{58}Fe}(p,n)\ce{^{58m}Co} reaction channel visibly peaking at approximately \mbox{20 MeV}.

This work presents the first measurement of several observables in this energy region including independent cross sections for      \ce{^{nat}Fe}(p,x)\ce{^{52m,g}Mn},   the  \ce{^{52\text{m}}Mn} ($2^+$) / \ce{^{52\text{g}}Mn}  ($6^+$)   isomer branching ratios via \ce{^{nat}Fe}(p,x),  as well as extending the \ce{^{nat}Fe}(p,x)\ce{^{52}Mn} excitation function down to the lowest energy to date. 
Cumulative  \ce{^{nat}Fe}(p,x)\ce{^{52g}Mn} reaction measurements exist in earlier work, but  this work presents the most well-characterized measurement  below 70\,MeV to date, with cross sections measured  at the 6--10\% uncertainty level.
This work is consistent with the general body of previous cumulative \ce{^{52g}Mn} measurements, with the exception of work from Williams, Lagunas-Solar, and Barchuk, all of which have shown discrepancies with prior data in other reaction channels measured here \cite{barchuk1987excitation,Lagunas-Solar1979a,PhysRev.162.1055}.


The \ce{^{52\text{m}}Mn} ($2^+$) / \ce{^{52\text{g}}Mn}  ($6^+$) ratio seen in Figure \autoref{fig:temp_52mMn} shows significant deviation from all of the reaction models and the TENDL database near threshold, indicating potential issues with transmission particle transmission coefficients arising from the optical model.  
...
% Cutting as part of results reorganization
%
...
% This is seen again in the  
For the \ce{^{nat}Fe}(p,x)\ce{^{52}Mn} cross section, seen in Figure \autoref{fig:temp_52Mn},  the  peak  at lower energy, which is largely attributable to the \ce{^{56}Fe}(p,p$\alpha$) channel, is well-modeled by TALYS and TENDL.
However, the production of \ce{^{52}Mn}  rises much more rapidly than the data would support above 50\,MeV, suggesting an overestimation of the \ce{^{56}Fe}(p,3p2n) contribution.  
...
Through measurement of the  \ce{^{nat}Fe}(p,x)\ce{^{52m}Mn} independent cross section
% , seen in Figure \autoref{fig:temp_52mMn}
, the effect of spin distributions in the unresolved discrete
% highly-excited nuclear 
states can be studied.
The measured data suggest that the independent cross section to the $2^+$ isomer should be a large fraction of the cumulative cross section to \ce{^{52}Mn} (which has a $6^+$ ground state), over 50\% at the peak energy.
...
% All three codes predict about 50\%, 
...
% Likewise, 
% Like \ce{^{51}Mn}, 
\ce{Fe}(p,x) offers an interesting production pathway for  \ce{^{52}Mn}.
...
\ce{^{nat}Fe}(p,x) offers a nearly threefold increase in production yield, but the lower radioisotopic purity (99.1\% for 20--30\,MeV, decreasing to 60.8\% by 40\,MeV) at higher energies due to the opening of \ce{^{54}Mn} makes this route seem impractical.
Much like \ce{^{51}Mn}, the use of an enriched \ce{^{54}Fe} target would prevent production of \ce{^{54}Mn} and long-lived Co radiocontaminants, providing a higher-yield production route over   \ce{^{nat}Cr}(p,x), with the tradeoff of necessitating higher-energy protons ($\leq$35\,MeV) for production.
It is important to note that the \ce{Fe}(p,x) route provides $\geq$60\% feeding of \ce{^{52g}Mn} ($t_{1/2}$ = 5.591$~\pm~$0.003\,d\,\cite{Dong2015}), implying that the short-lived  \ce{^{52m}Mn} ($t_{1/2}$ = 21.1$~\pm~$0.2\,m\,\cite{Dong2015}) can be easily separated through the difference in half-life, to avoid the 
% unfavorable gammas 
...
\subsection{\ce{^{nat}Fe}(p,x)\ce{^{56,58}Co} cross sections}
...
\begin{figure}[h]
 ...
 % Nb_ptallies.pdf: 380x298 pixel, 72dpi, 13.41x10.51 cm, bb=0 0 380 298
 \caption{Measured \ce{^{58\text{m}}Co} ($5^+$) / \ce{^{58\text{g}}Co}  ($2^+$)  isomer branching ratios via \ce{^{nat}Fe}(p,x), with the \ce{^{58}Fe}(p,n)\ce{^{58m}Co} reaction channel visibly peaking at approximately \mbox{20 MeV}.}
 \label{fig:58Co_IBR}
\end{figure}
...
% measurement of the 
\ce{^{nat}Fe}(p,x)\ce{^{58m}Co} reactions in the 0--70\,MeV region, 
the independent cross sections for      \ce{^{nat}Fe}(p,x)\ce{^{58g}Co}, and the \ce{^{58\text{m}}Co} ($5^+$) / \ce{^{58\text{g}}Co}  ($2^+$)  isomer branching ratios via \ce{^{nat}Fe}(p,x), seen in \autoref{fig:58Co_IBR}. 
The cumulative cross sections from these data are both consistent with, and higher-precision than, existing measurements of the cumulative \ce{^{nat}Fe}(p,x)\ce{^{58g}Co} cross sections, as well as the large body of measurements for \ce{^{nat}Fe}(p,x)\ce{^{56}Co}.

While it is dangerous to use weak channels to guide reaction modeling,
% Despite being weakly-fed, 
EMPIRE and TALYS/TENDL all reproduce earlier work and this measurement reasonably well ($\pm$10\%) for the \ce{^{nat}Fe}(p,x)\ce{^{58g}Co} reaction channel, as well as the strongly-fed \ce{^{nat}Fe}(p,x)\ce{^{56}Co} reaction.
This holds for both the shape and magnitude of each excitation function.
CoH overestimates the \ce{^{nat}Fe}(p,x)\ce{^{58g}Co} peak magnitude by approximately 30\% and  \ce{^{nat}Fe}(p,x)\ce{^{56}Co}  by approximately 10\%, despite matching the rising and falling edges of the curve.
For both the \ce{^{nat}Fe}(p,x)\ce{^{56,58g}Co} channels, ALICE underestimates the magnitude of the rising edge and compound peak by nearly 50\%, though it shows the best agreement of these codes with the magnitude of the pre-equilibrium tails for these channels.

However, all codes fail to reproduce the magnitude of the \ce{^{nat}Fe}(p,x)\ce{^{58m}Co} peak by approximately 25\% and completely fail to reproduce the pre-equilibrium tail for this channel, as measured in this work. 
% In 
As opposed to the other isomer-to-ground state ratio measured for Fe,  the opposite is seen for \ce{^{nat}Fe}(p,x)\ce{^{58}Co} -- the codes underpredict the ratio of the isomer-to-ground state, but in this case the isomer is the higher spin state ($5^+$, compared with a  $2^+$ ground state).
...
% as the main problem with modeling, or an underestimation of high-spin transfer 
...
The \ce{^{nat}Fe}(p,x)\ce{^{56}Co} channel ($t_{1/2}=77.236~\pm~0.026$ d, $\epsilon$=100\% to \ce{^{56}Fe}\,\cite{Junde2011})
% Several of the activities produced via \ce{^{nat}Fe}(p,x)  
...
Given that \ce{^{nat}Cu}(p,x)\ce{^{56}Co} is an established monitor reaction, it seems likely that an evaluation of \ce{^{nat}Fe}(p,x)\ce{^{56}Co} as a monitor standard could provide a useful complement to existing low-to-intermediate energy monitor reactions.
A similar argument can also be applied to \ce{^{nat}Fe}(p,x)\ce{^{58}Co}, which is another established copper monitor reaction product.
While this channel shares many of the same attractive features as \ce{^{nat}Fe}(p,x)\ce{^{56}Co}, the weak feeding of \ce{^{58}Co} makes this far less of an attractive option in comparison.
...
In addition to the \ce{^{nat}Fe}(p,x)\ce{^{51,52}Mn} measurements, this experiment has also yielded production cross section  measurements of  a number of additional  emerging radionuclides with medical applications,
...
Production of this radionuclide offers no major advantages over established pathways, with the  lower yield and radioisotopic purity failing to justify the convenience of natural targets  via   \ce{^{nat}Fe}(p,x).
...
While ingrowth of the long-lived \ce{^{58g}Co} ($t_{1/2}$ = 70.86(6) d\,\cite{Nesaraja2010}) is unavoidable, lowering the  co-production of \ce{^{58g}Co} is necessary to minimize patient dose.
...
\subsection{\ce{^{nat}Fe}(p,x)\ce{^{49, 51}Cr} cross sections}
...
%  % Nb_ptallies.pdf: 380x298 pixel, 72dpi, 13.41x10.51 cm, bb=0 0 380 298
%  \caption{Measured \ce{^{nat}Fe}(p,x)\ce{^{51}Cr} independent cross section, with the \ce{^{56}Fe}(p,$\alpha$pn)\ce{^{51}Cr}/\ce{^{54}Fe}(p,3pn)\ce{^{51}Cr} reaction channels visibly peaking at approximately \mbox{45 MeV}.}
%  \label{fig:51Cr_ind}
% \end{figure}
...
\begin{figure*}
    ...
    \subfloat{
        \centering
        \hspace{-5pt}\subfigimg[width=0.485\textwidth]{\hspace{4.5cm}a)}{./figures/49Cr.pdf}{50}
%         \caption{ Decay curve for the isomeric transition of \ce{^{115m}In}.}
         %         \refstepcounter{subfigure}
         \label{fig:fig49Cr}
   \hspace{-5pt}}%
     \subfloat{
        ...
        \subfigimg[width=0.485\textwidth]{\hspace{4.5cm}b)}{./figures/51Cr_ind.pdf}{50}
         %         \refstepcounter{subfigure}
         \label{fig:fig51Cr}
   \hspace{-5pt}}%
    \caption{a) Measured \ce{^{nat}Fe}(p,x)\ce{^{49}Cr} independent cross section, with the \ce{^{56}Fe}(p,$\alpha$p3n)\ce{^{49}Cr}/\ce{^{54}Fe}(p,3p3n)\ce{^{49}Cr} reaction channels visibly peaking at approximately \mbox{48 MeV}.   b) Measured \ce{^{nat}Fe}(p,x)\ce{^{51}Cr} independent cross section, with the \ce{^{56}Fe}(p,$\alpha$pn)\ce{^{51}Cr}/\ce{^{54}Fe}(p,3pn)\ce{^{51}Cr} reaction channels visibly peaking at approximately \mbox{45 MeV}.} 
     \label{fig:51Cr_ind}
\end{figure*}
...
This work presents the first measurement of \ce{^{nat}Fe}(p,x)\ce{^{49}Cr} and  the first independent cross sections for       \ce{^{nat}Fe}(p,x)\ce{^{51}Cr} in this energy region, seen in \autoref{fig:51Cr_ind}. 
The cumulative cross sections from these data are also consistent with existing measurements of the cumulative \ce{^{nat}Fe}(p,x)\ce{^{51}Cr} cross sections, and with improved precision.
...
While the \ce{^{51}Cr} decay data used in these experiments (Williams: $t_{1/2}=27.8$ d, $I_\gamma$=9\%, Schoen: $t_{1/2}=27.71$ d, $I_\gamma$=10.2\%) differ slightly from those used here ($t_{1/2}=27.704(3)$ d, $I_\gamma$=9.910(10)\% \cite{Wang2017}), these differences do not account for the approximately 50\% smaller peak cross section reported by Williams, and  30\%  smaller for Schoen. 
Neither publication reports any discussion of this channel, nor do they detail the length of each irradiation,  suggesting that transient equilibrium from the \ce{^{51}Mn} decay precursor was either not considered, or mis-calculated \cite{Basunia2020}.
% was not considered. 
 

 
For both the  \ce{^{49,51}Cr} channels, the shape is well-modeled by all codes in the energy range covered in this measurement, though ALICE appears to peak approximately 3\,MeV higher than the other codes.
For \ce{^{49}Cr}, ALICE and CoH both overestimate the magnitude of this channel by approximately 30\%, ALICE overestimates  by approximately 350\%, while TALYS and TENDL underestimate the magnitude by approximately 50\%.
% Weakly-fed channels such as \ce{^{49}Cr} are often sensitive, as the modeling of more strongly-fed channels tends to impose strong constraints upon the magnitude of remaining channels.
...
Modeling of \ce{^{51}Cr} shows improved agreement in magnitude --- CoH and ALICE are consistent at the 1$\sigma$ level, EMPIRE overestimates the peak by approximately 25\%, and TALYS/TENDL underestimate the peak by approximately 60\%.
However, this level of disagreement is more concerning than for \ce{^{49}Cr}, due to the strong feeding of this channel.
...
\subsection{\ce{^{nat}Fe}(p,x)\ce{^{54, 56}Mn} cross sections} 
...
%  % Nb_ptallies.pdf: 380x298 pixel, 72dpi, 13.41x10.51 cm, bb=0 0 380 298
%  \caption{Measured \ce{^{nat}Fe}(p,x)\ce{^{54}Mn} independent cross section, with the \ce{^{56}Fe}(p,2pn)\ce{^{54}Mn}/\ce{^{57}Fe}(p,2p2n)\ce{^{54}Mn} reaction channels visibly peaking at approximately \mbox{40 MeV}.}
%  \label{fig:54Mn}
% \end{figure}
...
\begin{figure*}
    ...
    \subfloat{
        \centering
        \hspace{-5pt}\subfigimg[width=0.485\textwidth]{\hspace{4.5cm}a)}{./figures/54Mn.pdf}{50}
%         \caption{ Decay curve for the isomeric transition of \ce{^{115m}In}.}
         %         \refstepcounter{subfigure}
         \label{fig:fig54Mn}
   \hspace{-5pt}}%
     \subfloat{
        ...
        \subfigimg[width=0.485\textwidth]{b)}{./figures/56Mn.pdf}{50}
         %         \refstepcounter{subfigure}
         \label{fig:fig56Mn}
   \hspace{-5pt}}%
    \caption{a) Measured \ce{^{nat}Fe}(p,x)\ce{^{54}Mn} independent cross section, with the sum of the \ce{^{56}Fe}(p,2pn)\ce{^{54}Mn} and \ce{^{57}Fe}(p,2p2n)\ce{^{54}Mn} reaction channels visibly peaking at approximately \mbox{40 MeV}.   b) Measured \ce{^{nat}Fe}(p,x)\ce{^{56}Mn} independent cross section, with the \ce{^{57}Fe}(p,2p)\ce{^{56}Mn}/\ce{^{58}Fe}(p,2pn)\ce{^{56}Mn} reaction channels visibly peaking at approximately \mbox{30 MeV} and \mbox{40 MeV}, respectively.} 
     \label{fig:54Mn}
\end{figure*}
...
This work presents the first measurements the \ce{^{nat}Fe}(p,x)\ce{^{56}Mn} reaction below 70\,MeV.
Additionally, we report the highest precision measurement of \ce{^{nat}Fe}(p,x)\ce{^{54}Mn} in this energy region.
...
Modeling of these channels presents a counterpoint to the \ce{^{51,52}Mn} results discussed earlier --- \ce{^{54}Mn} is   well-modeled by CoH, TENDL, and TALYS, though EMPIRE overestimates the peak cross section by approximately 25\%, and ALICE underestimates by the same, in addition to peaking approximately 3\,MeV higher.
The weakly-fed \ce{^{56}Mn} is poorly modeled in comparison, with CoH, EMPIRE, and ALICE failing to match neither the shape nor magnitude of the excitation function, though TALYS/TENDL matches both.
While the poorly-modeled \ce{^{51,52}Mn} channels are dominated by alpha particle emission,  \ce{^{54}Mn} production is more characterized by the the emission of the constituent protons and neutrons.
Since TALYS, TENDL, and CoH exhibited poor predictive power for \ce{^{51,52}Mn}, but reproduced \ce{^{54}Mn} well, this observation would suggest that the alpha optical models employed in these codes are underestimating the strength of alpha emission in this energy region. 
...
% % This work reports the first experimental measurement of
% this mass region, 
% % including the \ce{^{nat}Fe}(p,x)\ce{^{49}Cr}, 
% % \ce{^{nat}Fe}(p,x)\ce{^{51}Mn}, \ce{^{nat}Fe}(p,x)\ce{^{52m}Mn},  
% % \ce{^{nat}Fe}(p,x)\ce{^{56}Mn}, 
% % and \ce{^{nat}Fe}(p,x)\ce{^{58m}Co} reactions in the 0--70\,MeV region, 
% % It also represents  the first measurement of 
% the independent cross sections for      
% % \ce{^{nat}Fe}(p,x)\ce{^{51}Cr}, \ce{^{nat}Fe}(p,x)\ce{^{52g}Mn}, 
% % \ce{^{nat}Fe}(p,x)\ce{^{58g}Co}, 
% % and the \ce{^{52\text{m}}Mn} ($2^+$) / \ce{^{52\text{g}}Mn}  ($6^+$) and \ce{^{58\text{m}}Co} ($5^+$) / \ce{^{58\text{g}}Co}  ($2^+$)  isomer branching ratios via \ce{^{nat}Fe}(p,x).  
...
% In particular, \ce{^{nat}Fe}(p,x)\ce{^{56}Co} ($t_{1/2}=77.236~\pm~0.026$ d, $\epsilon$=100\% to \ce{^{56}Fe}\,\cite{Junde2011})  is a strongly-fed reaction channel (peak cross section of approximately 400\,mb near 12\,MeV), with a sufficiently-long half-life permitting offline gamma spectrometry,  and  cannot be populated via secondary neutrons incident upon the monitor target. 
% It possesses a number of intense gamma-rays, which are distinct from those populated by daughter states in the decay of  \ce{^{56}Mn}, and the gammas which are produced both in the decay of  \ce{^{56}Co} and  \ce{^{56}Mn} may be easily resolved based on differences in intensity and half-life. 
Similarly to \ce{^{56}Co},  \ce{^{nat}Fe}(p,x)\ce{^{54}Mn}  offers another interesting possibility as a \ce{^{nat}Fe}(p,x) monitor reaction,
with a long half-life 
% This radionuclide 
($t_{1/2}=312.20~\pm~0.020$ d, $\epsilon$=100\% to \ce{^{54}Cr}\,\cite{Dong2014}),
% has a convenient half-life and 
strong cross section (approximately 160\,mb near 40\,MeV), and a uniquely-produced daughter nuclide.
% , and is immune from two different reactions on the same monitor foil leading to states in the same daughter nuclide.
This reaction could be useful for intermediate- to high-energy protons ($E_p \geq$30\,MeV), but would be susceptible to production via  high-energy secondary neutrons  (threshold 12.1\,MeV off of \ce{^{56}Fe}).
% produced in these facilities, though (p,x) production rates should dominate (n,x) through both particle flux and cross section. 
Additionally, the \ce{^{nat}Fe}(p,x)\ce{^{54}Mn} and \ce{^{nat}Fe}(p,x)\ce{^{56}Co} channels have a difference in apparent energetic thresholds,
% of nearly 20\,MeV, 
providing some energy discrimination sensitivity in the same iron foil, particularly in the 20--50\,MeV region.
% , which could be useful for determining incident proton energy near 40\,MeV, where fast neutrons are less significant than deeper in the stack.
...
\subsection{\ce{^{nat}Cu}(p,x)\ce{^{60,61,64}Cu} cross sections} 

\begin{figure}[h]
 \centering
 \includegraphics[width=0.5\textwidth]{./figures/64Cu.pdf}
 % Nb_ptallies.pdf: 380x298 pixel, 72dpi, 13.41x10.51 cm, bb=0 0 380 298
 \caption{Measured \ce{^{nat}Cu}(p,x)\ce{^{64}Cu} independent cross section, with the \ce{^{65}Cu}(p,pn)\ce{^{64}Cu} reaction channels visibly peaking at approximately \mbox{25 MeV}.}
 \label{fig:64Cu}
\end{figure}

As with many \ce{^{nat}Cu}(p,x) channels, these reactions have been previously measured in our earlier work, but this measurement extends these cross sections down to reaction thresholds and reports cross sections for many of the short-lived products unable to be observed previously, such as \ce{^{60}Cu} \cite{Voyles2018a}.
For \ce{^{nat}Cu}(p,x)\ce{^{60,61,64}Cu}, the measurements presented here are consistent with earlier work (aside from that of Williams \cite{PhysRev.162.1055}, as discussed earlier), and are generally the highest-precision measurements to date.
As part of this work, a discrepancy, outside reported uncertainties, has been observed between the accepted and measured  values of the intensity ratio of the 283 keV and  656 keV decay lines in \ce{^{61}Cu}, and is detailed in a forthcoming manuscript.




As we reported previously, modeling of \ce{^{nat}Cu}(p,x) reactions is poor in this energy region.
% , despite the relative ease of only two stable target isotopes in copper.
For \ce{^{60}Cu}, all codes fail to predict the magnitude and shape of the channel, with a peak approximately 5\,MeV lower than observed.
Similar behavior is observed in the strongly-fed \ce{^{61}Cu},
% , though not to the same degree --- 
with the peak shifted approximately 3\,MeV lower.
% , but the shape predictions are more physical, despite a suppression of the pre-equilibrium tail in  EMPIRE.
\ce{^{64}Cu} modeling appears to be the best of these channels, with all codes matching the overall shape, but only EMPIRE predicting the rapid rise and fall of the compound peak.
% , compared to the smoother behavior of the other codes.
Unlike ALICE and EMPIRE, CoH, TALYS, and TENDL all do a successful job of modeling the shape of the (p,pn) pre-equilibrium tail, with this channel showing the best predictive power of the \ce{^{nat}Cu}(p,x) reactions.
As seen with ALICE for \ce{^{61}Cu}/\ce{^{64}Cu}, the alternating underestimation/overestimation across channels for a single code points to competition between neighboring reaction channels as a common issue in this mass region.




% As with the other foils in this experiment, 
\ce{^{nat}Cu}(p,x)  also  yielded measurements of  a number of additional  medical  radionuclides,  including the non-standard positron emitters 
\ce{^{57}Ni}\,\cite{PMID:7632762,zweit1996medium,Graves2016,Rosch2014}, 
\ce{^{61}Cu}\,\cite{Szelecsenyi2005a,Fukumura2004}, and
\ce{^{64}Cu}\,\cite{Lewis2003,Bandari2014,mp500671j,Szelecsenyi1993,Aslam2009,Hilgers2003,Szelecsenyi2005,Voyles2017}.
However, production of these radionuclides  offers no major advantages over established  pathways, with the  lower yield and radioisotopic purity failing to justify the convenience of natural targets  via    \ce{^{nat}Cu}(p,x). 
...
\subsection{\ce{^{nat}Ti}(p,x)\ce{^{44,47,48}Sc} cross sections} 
...
\begin{figure}[h]
 ...
 % Nb_ptallies.pdf: 380x298 pixel, 72dpi, 13.41x10.51 cm, bb=0 0 380 298
 \caption{Measured \ce{^{44\text{m}}Sc} ($6^+$) / \ce{^{44\text{g}}Sc}  ($2^+$)  isomer branching ratios via \ce{^{nat}Ti}(p,x).}
 \label{fig:44Sc_IBR}
\end{figure}



% Due in part to the long-standing use of Ti as a monitor foil,  \ce{^{nat}Ti}(p,x) reactions are better-characterized than many  natural targets.
While the focus of this work is on the production of \ce{^{51,52}Mn}  and other \ce{^{nat}Fe}(p,x) channels, the cross sections reported here offer improvements to prior work, particularly for the several Sc radionuclides observed. 
For all of the \ce{^{nat}Ti}(p,x)\ce{^{44,47,48}Sc} channels, this work is consistent with all prior data and is generally the highest-precision measurement to date.
This work additionally  extends the \ce{^{44g}Sc} excitation function to the highest energy to date. 



% Perhaps due to the abundance of reaction data available, 
The modeling of these channels is markedly improved  in comparison to the \ce{^{nat}Fe}(p,x) channels discussed previously.
For \ce{^{44m}Sc}, 
% the shape of the excitation function is well-modeled by EMPIRE, CoH, TALYS, and TENDL, though 
...
The same modeling trend is observed for \ce{^{44m}Sc}, 
% though no judgment can be made for the pre-equilibrium tail, due to a paucity of data above 50\,MeV.
and reflected in the measured isomer branching ratios for \ce{^{44}Sc}, seen in \autoref{fig:44Sc_IBR}.
Above 23\,MeV, TALYS/TENDL both reproduce the present work, as well as earlier measurements, suggesting that angular momentum transfer in the pre-equilibrium regime is being well-modeled, but that the total feeding of the \ce{^{44}Sc} channel is  underestimated. 
...
% , suggesting a more fundamental issue with the modeling of this channel. 
\ce{^{47}Sc} presents a very opposed situation, despite an abundance of reaction data and strong feeding of the channel.
...
As a cumulative reaction channel, it is unclear whether this failure arises from modeling of any \ce{^{47}Ca} decay precursors, or the competition between  \ce{^{48}Ti}(p,20)\ce{^{47}Sc} / \ce{^{49}Ti}(p,2pn)\ce{^{47}Sc} / \ce{^{50}Ti}(p,$\alpha$)\ce{^{47}Sc} channels. 
Modeling of \ce{^{48}Sc} is lacking, with only TALYS/TENDL matching the shape of the excitation, and all codes failing to reproduce the magnitude of the channel.




As with \ce{^{58m}Co} measurements, the titanium activation has  yielded measurements of  a number of additional  medical  radionuclides,  including the non-standard positron emitter
\ce{^{44}Sc}\,\cite{Muller2013,Filosofov2010,Qaim2011}
%\,\cite{Qaim2011,Qaim201731,Kolsky1998,mausner1995evaluation}.
% \ce{^{86}Y}\,\cite{Valdovinos2017,Nickles2003,Qaim2008,QaimSyedM2011,Rosch1993,doi:10.1139/v67-193,levkovski1991cross,Johnson2015,Singh2013,Kiselev1974,Kandil2009}, 
% \ce{^{89}Zr}\,\cite{Verel2003,Dijkers2009,Dijkers2010,PhysRevC.38.1624,Omara2009},  
...
\ce{^{47}Sc}\,\cite{Muller2014,Deilami-nezhad2016}.
% and the Auger-therapy agent \ce{^{82\text{m}}Rb}\,\cite{Kovacs1991,Titarenko2011}. 
% Production of \ce{^{44}Sc} 
However, this route offers no major advantages over established \ce{^{44}Ca}(p,n) / \ce{^{44}Ca}(d,2n) / \ce{^{45}Sc}(p,2n)\ce{^{44}Ti} $\xrightarrow[]{\epsilon}$ \ce{^{44}Sc} pathways.
% , with the  lower yield and radioisotopic purity failing to justify the convenience of natural targets  via    \ce{^{nat}Ti}(p,x). 
Since high-specific activity \ce{^{47}Sc} production routes are limited (with  \ce{^{47}Ti}(n,p)\ce{^{47}Sc} as one of few potential candidates), alternative pathways are greatly desired by the community.
However, \ce{^{nat}Ti}(p,x) is clearly not a compelling alternative if high specific activity is required, due to the inability to avoid co-production of long-lived Sc radiocontaminants.
% , even with prudent selection of a beam energy window.
...
% \textred{ As shown in Fig. 6, the bell-like shape of the cross section in the (p,n) reaction is mainly determined by Q-values of the (p,n) and (p,pn) reactions and level densities of nuclei as follows; Threshold of the (p,n) cross section is determined by the Q-value of the (p,n) reaction. The Q-value of the (p,pn) reaction determines decline and peak of the (p,n) cross section. The height of the (p,n) cross section is governed by the probabilities of the exit channels from the compound nucleus, namely (p,gamma), (p,n), (p,alpha) and (p,pn). These probabilities are connected to the level densities of the nuclei in the exit channels. Additionally, the (p,2n) and (p,p2n) reactions compete mainly with the (p,pn) one.}
...
Clearly, the use of \ce{Fe}(p,x)\ce{^{51,52}Mn} has significant untapped potential, and additional work is needed to further characterize these reaction channels for $E_p \leq$60\,MeV, as well as the potential for \ce{^{nat}Fe}(p,x)\ce{^{54}Mn},\ce{^{56}Co} as future monitor reactions.
