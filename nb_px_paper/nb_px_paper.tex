...
A set of 38 measurements of cross sections for the \ce{^{nat}Nb}(p,x) and  \ce{^{nat}Cu}(p,x) reactions between 40.34 -- 89.74 MeV are presented in this work, as well as 5 independent measurements of isomer branching ratios.
Cross sections were measured using the well-characterized \ce{^{nat}Al}(p,x)\ce{^{22}Na}, \ce{^{nat}Al}(p,x)\ce{^{24}Na}, \ce{^{nat}Cu}(p,x)\ce{^{56}Co}, \ce{^{nat}Cu}(p,x)\ce{^{62}Zn}, and \ce{^{nat}Cu}(p,x)\ce{^{65}Zn} monitor reactions  for accurate proton fluence measurement, with all activities quantified using high-purity germanium spectroscopy.
...
\comment{Stephen: Unless you've already discussed this with co-authors, I would have someone from LANL (i.e. Meiring) instead or in addition.  Add one following discussion.}
